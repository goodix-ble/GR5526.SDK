...
lv_obj_t *lv_layout_app_grid_create(lv_obj_t *parent, lv_enhanced_menu_item_t *items, uint16_t item_count, uint8_t icon_per_row, bool use_triform)
{
    ...
    if (use_triform)
    {
        ...
        lv_enhanced_grid_set_cell_size(grid, 90, 110);
        lv_enhanced_grid_set_padding(grid, 45, DISP_VER_RES / 3);
        lv_point_t scroll_offset = {45, (DISP_VER_RES - 100) / 2};
    ...
    else
    {
        ...
        lv_enhanced_grid_set_cell_size(grid, 120, 120);
        lv_enhanced_grid_set_padding(grid, DISP_HOR_RES / 2 - 65, DISP_VER_RES / 2 - 65);
...
#define SPHERE_RADIUS     500.f
#define DISTORTION_COEFF  1.3f
...
static float app_grid_triform_translate_cb(lv_point_t *icon_pos, const lv_enhanced_menu_item_t *item)
{
#define SCREEN_RADIUS 170.f
    float dist = fabsf(icon_pos->y - 180.f);
    float scale = 0;
    if (dist < SCREEN_RADIUS)
    {
        scale = 1.35f * sqrtf(SQUARE(SCREEN_RADIUS) - SQUARE(dist)) / SCREEN_RADIUS;
    }
    ...
    if (icon_pos->x != 180)
    {
        icon_pos->x += xofs * copysignf(1.f, icon_pos->x - 180.f);
    }
