...
static void lv_clock_update_angle_cb(lv_anim_t* anim){
    ...
    if(hands_obj[SEC_HAND_INDEX] != NULL)
        lv_img_set_angle(hands_obj[SEC_HAND_INDEX], sec_angle);
    if(hands_obj[MIN_HAND_INDEX] != NULL)
        lv_img_set_angle(hands_obj[MIN_HAND_INDEX], min_angle);
    if(hands_obj[HOUR_HAND_INDEX] != NULL)
        lv_img_set_angle(hands_obj[HOUR_HAND_INDEX], hour_angle);
}
...
void lv_clk_hand_stop_run(void){
    ...
    lv_anim_del(&clock_anim, lv_clock_anim_cb);
}
