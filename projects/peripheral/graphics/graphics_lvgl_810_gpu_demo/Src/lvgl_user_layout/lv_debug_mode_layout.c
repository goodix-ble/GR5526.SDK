...
extern void lv_port_debug_info_enable(bool enable);
...
static void event_handler(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED){
        ...
        switch(lv_roller_get_selected(obj)) {
            ...
            case 1:
            {
                lv_port_debug_info_enable(false);
            }
            break;
            default:
            {
...
void lv_debug_mode_setting_view_destroy(void)
{
...
lv_obj_t* lv_debug_mode_setting_view_create(void)
{
    if(NULL == switch_layout){
        ...
        lv_label_set_text_fmt(lab, "调试模式");
    ...
    lv_wms_self_destroy_func_set(switch_layout, lv_debug_mode_setting_view_destroy);
