                      ...
                      const mbedtls_md_context_t *src )
{
    ...
    switch( src->md_info->type )
    {
        ...
        case MBEDTLS_MD_SHA256:
#if defined(HAS_SHA_DRIVER)
            crypto_sha256_clone( dst->md_ctx, src->md_ctx );
#else
            mbedtls_sha256_clone( dst->md_ctx, src->md_ctx );
#endif
            break;
...
int mbedtls_md_setup( mbedtls_md_context_t *ctx, const mbedtls_md_info_t *md_info, int hmac )
{
    ...
    switch( md_info->type )
    {
        ...
        case MBEDTLS_MD_SHA256:
#if defined(HAS_SHA_DRIVER)
            ctx->md_ctx = mbedtls_calloc( 1, sizeof( crypto_sha256_context ) );
            if( ctx->md_ctx == NULL )
                return( MBEDTLS_ERR_MD_ALLOC_FAILED );
            crypto_sha256_init( ctx->md_ctx );
#else
            ALLOC( sha256 );
#endif
            break;
...
int mbedtls_md_starts( mbedtls_md_context_t *ctx )
{
    ...
    switch( ctx->md_info->type )
    {
        ...
        case MBEDTLS_MD_SHA256:
#if defined(HAS_SHA_DRIVER)
            return( crypto_sha256_starts( ctx->md_ctx ) );
#else
...
int mbedtls_md_update( mbedtls_md_context_t *ctx, const unsigned char *input, size_t ilen )
{
    ...
    switch( ctx->md_info->type )
    {
        ...
        case MBEDTLS_MD_SHA256:
#if defined(HAS_SHA_DRIVER)
            return( crypto_sha256_update( ctx->md_ctx, input, ilen ) );
#else
            return( mbedtls_sha256_update_ret( ctx->md_ctx, input, ilen ) );
#endif
#endif
...
int mbedtls_md_finish( mbedtls_md_context_t *ctx, unsigned char *output )
{
    ...
    switch( ctx->md_info->type )
    {
        ...
        case MBEDTLS_MD_SHA256:
#if defined(HAS_SHA_DRIVER)
            return( crypto_sha256_finish( ctx->md_ctx, output ) );
#else
            return( mbedtls_sha256_finish_ret( ctx->md_ctx, output ) );
#endif
#endif
            ...
            unsigned char *output )
{
    ...
    switch( md_info->type )
    {
        ...
        case MBEDTLS_MD_SHA256:
#if defined(HAS_SHA_DRIVER)
            return( crypto_sha256( input, ilen, output ) );
#else
...
int mbedtls_md_hmac_starts( mbedtls_md_context_t *ctx, const unsigned char *key, size_t keylen )
{
    ...
    if (ctx->md_info->type == MBEDTLS_MD_SHA256)
    {
        return ( crypto_hmac_sha256_starts(ctx->md_ctx, key, keylen) );
    }
#endif
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
int mbedtls_md_hmac_update( mbedtls_md_context_t *ctx, const unsigned char *input, size_t ilen )
{
    ...
    if (ctx->md_info->type == MBEDTLS_MD_SHA256)
    {
        return ( crypto_hmac_sha256_update(ctx->md_ctx, input, ilen) );
    }
#endif
    if( ctx == NULL || ctx->md_info == NULL || ctx->hmac_ctx == NULL )
        return( MBEDTLS_ERR_MD_BAD_INPUT_DATA );
...
int mbedtls_md_hmac_finish( mbedtls_md_context_t *ctx, unsigned char *output )
{
    ...
    if (ctx->md_info->type == MBEDTLS_MD_SHA256)
    {
        return ( crypto_hmac_sha256_finish(ctx->md_ctx, output) );
    }
#endif
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
                     ...
                     unsigned char *output )
{
    ...
    if (md_info->type == MBEDTLS_MD_SHA256)
    {
        return ( crypto_hmac_sha256(key, keylen, input, ilen, output) );
    }
#endif
    mbedtls_md_context_t ctx;
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
