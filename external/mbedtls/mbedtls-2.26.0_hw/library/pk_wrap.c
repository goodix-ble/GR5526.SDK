                   ...
                   const unsigned char *sig, size_t sig_len )
{
    ...
    uint8_t zero_padding_N = mbedtls_mpi_size(&rsa->N) % 32;
    if (zero_padding_N == 0)
        mbedtls_mpi_write_binary( &rsa->N, N_buf, mbedtls_mpi_size(&rsa->N) );
    else
        mbedtls_mpi_write_binary( &rsa->N, N_buf+32-zero_padding_N, mbedtls_mpi_size(&rsa->N) ); //avoid omitted zero
    ...
    if ((ret = (int)crypto_rsa_pkcs1_verify(&rsa->rsa_config, sig_len*8, (uint8_t *)hash,
                                            hash_len, e, (uint32_t *)N_buf, NULL, constp, sig_buf, 1)) != 0)
#else
                   ...
                   int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    ...
    *sig_len = mbedtls_rsa_get_len( rsa );
    ...
    uint8_t D_buf[256] = {0};
    uint8_t zero_padding_N = mbedtls_mpi_size(&rsa->N) % 32;
    uint8_t zero_padding_D = mbedtls_mpi_size(&rsa->D) % 32;
    if (zero_padding_N == 0)
        mbedtls_mpi_write_binary( &rsa->N, N_buf, mbedtls_mpi_size(&rsa->N) );
    else
        mbedtls_mpi_write_binary( &rsa->N, N_buf+32-zero_padding_N, mbedtls_mpi_size(&rsa->N) ); //avoid omitted zero
    if (zero_padding_D == 0)
        mbedtls_mpi_write_binary( &rsa->D, D_buf, mbedtls_mpi_size(&rsa->D) );
    else
        mbedtls_mpi_write_binary( &rsa->D, D_buf+32-zero_padding_D, mbedtls_mpi_size(&rsa->D) ); //avoid omitted zero
    swap_endian((uint32_t *)N_buf, (*sig_len)/4, (uint32_t *)N_buf);
    swap_endian((uint32_t *)D_buf, (*sig_len)/4, (uint32_t *)D_buf);
    return (int)crypto_rsa_pkcs1_sign( &rsa->rsa_config , (*sig_len)*8, (uint8_t *)hash, hash_len,
