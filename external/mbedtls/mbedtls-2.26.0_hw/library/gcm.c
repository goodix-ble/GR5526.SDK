...
{
#if defined(HAS_AES_DRIVER)
    return crypto_gcm_setkey( ctx, key, keybits );
#else
...
{
#if defined(HAS_AES_DRIVER)
    return crypto_gcm_starts( ctx, mode, iv, iv_len, add, add_len );
#else
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
{
#if defined(HAS_AES_DRIVER)
    return crypto_gcm_update( ctx, length, input, output );
#else
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
{
#if defined(HAS_AES_DRIVER)
    return crypto_gcm_finish( ctx, tag, tag_len );
#else
    unsigned char work_buf[16];
    size_t i;
    uint64_t orig_len;
...
{
#if defined(HAS_AES_DRIVER)
    return crypto_gcm_crypt_and_tag( ctx, mode, length, iv, iv_len, add, add_len, input, output, tag_len, tag );
#else
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
{
#if defined(HAS_AES_DRIVER)
    return crypto_gcm_auth_decrypt( ctx, length, iv, iv_len, add, add_len, tag, tag_len, input, output );
#else
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
